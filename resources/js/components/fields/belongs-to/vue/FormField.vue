...
export default {
    mixins: [Affects, TogglesTrashed, PerformsSearches, HandlesValidationErrors],
    props: {
        resourceName: String,
        field: Object,
        viaResource: {},
     ...
     */
    mounted() {
        this.startEmpty = this.field.startEmpty || false;
        if(!this.startEmpty)
    ...
    watch: {
        ...
        selectedResourceId: function(value) {
            if(!this.isSearchable){
                console.log('emitting from ' + this.field.attribute)
                // Trigger onchange event for any child attribute.
                Nova.$emit('value-affected-' + this.field.attribute, {
                    value: value,
                    field_values: this.getFieldValues()
